---
layout: post
title:  "Angular with Devise and Rails Walkthrough"
...
Here is the GitHub repo for this walkthrough: 
...
Open your Gemfile, remove `gem ‘turbolinks’` (because it causes problems with javascript and jQuery) and add the following:
...
By the time you’ve got momentum building out your app, you’ll likely have many more dependencies or “packages” but here’s what you’ll need to get started. Add the following vendor dependencies to `bower.json` in your root directory:
...
Before I overwhelm you with the AuthCtrl, I just want to point out that most of what you’re seeing above is Bootstraped CSS classes so that you’re all super impressed with how beautifully this renders. Ignore all of the class attributes, and everything else should be pretty familiar, such as `ng-submit`, `ng-model`, and `ui-sref`, which takes the places of our usual `href` a-tag attribute. Now for the `AuthCtrl`… are you ready?
